node {
    ...
    stage('Preparation') {
        checkout([$class: 'GitSCM', branches: [[name: '*/comm_dev']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'uniskare_remote_server', url: 'https://github.com/KHAKhazeus/Uniskare/']]])
    }
    ...
    stage('DockerPush') {
        ...
        sh """
            ...
            sudo docker tag uniskare_eureka_comm/eureka_comm registry.cn-shanghai.aliyuncs.com/uniskare/comm:1.0
            sudo docker push registry.cn-shanghai.aliyuncs.com/uniskare/comm:1.0
            """
        echo '================结束推送镜像================'
    ...
    stage('RemotePull'){
        ...
        sh """
            ...
            sudo docker run -itd -p 8925:8925 --rm --network=host --name=comm-container registry.cn-shanghai.aliyuncs.com/uniskare/comm:1.0 --spring.profiles.active=peer1
            echo 'finished!'
            exit
            remotessh
        """
