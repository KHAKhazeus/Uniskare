node {
    ...
    stage('RemotePull'){
        ...
        sh """
            ...
            sudo docker stop order-container-2
            sudo docker stop order-container-3
            sudo docker rm order-container-1
            sudo docker rm order-container-2
            sudo docker rm order-container-3
            echo '========停止并删除旧的容器成功============='
            sudo docker login --username=柠檬一起啊啊啊啊啊 --password=woshi123 registry.cn-shanghai.aliyuncs.com
            ...
            sudo docker run -itd -p 8910:8910 --rm --network=host --name=order-container-2 registry.cn-shanghai.aliyuncs.com/uniskare/order:1.0 --spring.profiles.active=peer2
            sudo docker run -itd -p 8911:8911 --rm --network=host --name=order-container-3 registry.cn-shanghai.aliyuncs.com/uniskare/order:1.0 --spring.profiles.active=peer3
            echo 'finished!'
            exit
            remotessh
        """
